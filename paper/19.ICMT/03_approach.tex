...
Although the rules should obey certain scheduling, \eg killing has priority over moving to end the game, in this work we assume a graph grammar \ie any rule can be applied at any time during the execution of the transformation\es{do we need this assumption?}.
...
%Formally, we consider a model to be a labeled, attributed multi-graph.
%We extract a subgraph to represent the layout of the positions.
%The nodes in the layout subgraph are the position objects and the edges are the references between them along which movable objects can move.
%In the Pacman example, grid nodes are the nodes for position objects.
%The edges correspond to the \texttt{left}, \texttt{right}, \texttt{up}, and \texttt{down} references between them.
%
%\es{Formal notation if needed:}
...
The move distance of a movable object as the length of the shortest path from its position in model M1 to its position in model M2.
